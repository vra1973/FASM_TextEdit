...
MAXSIZE equ 260 ; maximum file name in bytes
MEMSIZE equ 65537 ; temporary buffer size

section '.data' data readable writeable

title db 'Simple Text Editor. Vasylenko Roman (2021)v01',0
class db 'FASMWIN32',0
edit db 'EDIT',0
saveq db 'Save changes ?',0
filter db 'Text Files',0,'*.txt',0
db 'All formats',0,'*.txt;*.asm;*.inc;*.ini',0
db 'All files',0,'*.*',0,0
fn_saved db 0 ; filename flag 1 = saved, 0 = not
errtxt db 'Error code: %u',0
errbuf rb $-errtxt+10
fname rb MAXSIZE
hfile dd ?
hheap dd ?
...
jmp .finish
.wmcommand:  ; event handling of the selected menu
mov eax,[wparam]
cmp ax,IDM_NEW
je .NEW
cmp ax,IDM_OPEN
...
.wmcreate:     ; window creation event handling
...
menuitem <'&New',9,'Ctrl+N'>,IDM_NEW,0
menuitem <'&Open',9,'Ctrl+O'>,IDM_OPEN,0
menuitem <'&Save',9,'Ctrl+S'>,IDM_SAVE,0
menuitem 'Save &as',IDM_SAVEAS,0
menuseparator
menuitem <'&Quit',9,'Ctrl+Q'>,IDM_EXIT,MFR_END

menuitem '&Edit',0,MFR_POPUP
menuitem <'&UnDo',9,'Ctrl+Z'>,IDM_UNDO
menuseparator
menuitem <'&Cut',9,'Ctrl+X'>,IDM_CUT
menuitem <'&Copy',9,'Ctrl+C'>,IDM_COPY
menuitem <'&Insert',9,'Ctrl+V'>,IDM_PASTE
menuitem <'&Delete',9,'Del'>,IDM_DELETE
menuseparator
menuitem <'Select &all',9,'Ctrl+A'>,IDM_SELECTALL,MFR_END
...
dialog about_dialog,'About the program',40,40,172,60,WS_CAPTION+WS_POPUP+WS_SYSMENU+DS_MODALFRAME
dialogitem 'STATIC',<'Simple Text Editor',0Dh,0Ah,'Vasylenko Roman Andreevich. 2021.v01.'>,-1,27,10,144,40,WS_VISIBLE+SS_CENTER
dialogitem 'STATIC',IDI_MAIN,-1,8,8,32,32,WS_VISIBLE+SS_ICON
dialogitem 'STATIC','',-1,4,34,164,11,WS_VISIBLE+SS_ETCHEDHORZ
dialogitem 'STATIC','Written with the FASM compiler',-1,12,42,100,20,WS_VISIBLE+SS_LEFT
